...
# TP 7 - BERT pour l’analyse de sentiment : Finetuning sur des News financières
"""
...
"""1. Harmonisation des labels entre les deux jeux de données"""
...
"""2. Fonction d'entraînement, d'évaluation et de sauvegarde d'un modèle de classification de texte
...
def train_model(model_name, dataset, batch_size=16, num_epochs=3, save_path=None):
    ...
    print("🚀 Sauvegarde du modèle dans :", save_path)
    model.save_pretrained(save_path)
    tokenizer.save_pretrained(save_path)

    print("📂 Contenu du dossier après sauvegarde :", os.listdir(save_path))
...
#vérifier que le drive est bien monté sur colab
import os
print("Drive monté ici :", os.path.exists("/content/drive/MyDrive"))

"""3. Finetuning des modèles"""
...
Perte (Loss) : Le modèle BERT a une perte d'évaluation légèrement plus faible (0.3566) par rapport à FinBERT (0.3591), ce qui indique qu’il a mieux appris à minimiser l'erreur lors de l'entraînement.

Accuracy : Le modèle BERT obtient une précision de 86.45%, tandis que FinBERT a une précision de 85.90%. Cela montre que BERT fait légèrement plus de prédictions correctes que FinBERT.

Précision : BERT a une précision de 86.31%, qui est également un peu meilleure que celle de FinBERT (86.12%). Cela signifie que BERT a un peu moins de faux positifs que FinBERT.

Rappel (Recall) : Le rappel de BERT est aussi 86.45%, ce qui est supérieur à celui de FinBERT qui est de 85.90%. Cela montre que BERT a mieux réussi à capturer les cas positifs que FinBERT.

Score F1 : Le score F1 de BERT est de 86.31%, tandis que celui de FinBERT est de 85.99%. Le score F1 combine la précision et le rappel, et il est également un peu meilleur pour BERT.

Temps d'exécution (Runtime) : Les temps d'exécution sont très similaires, avec BERT prenant 4.96 secondes et FinBERT prenant 4.99 secondes, ce qui montre que les deux modèles sont assez rapides.

Nombre d'exemples traités par seconde : BERT traite 583 exemples par seconde, tandis que FinBERT traite 580 exemples par seconde. Là aussi, les performances sont très proches.

Conclusion : Le modèle BERT surpasse légèrement FinBERT sur toutes les métriques de performance. Bien que les différences soient minimes, BERT semble être plus efficace pour cette tâche d'évaluation. Toutefois, FinBERT, étant un modèle spécialisé pour le domaine financier, peut avoir des avantages pour des cas spécifiques liés à la finance.
"""